...
## 마지막 업데이트: 2026-07-08 (입력창 다크모드 통일 + 홈 히어로 재배치 + 이미지/텍스트 일정 등록 에이전트 + 홈 섹션 드래그앤드롭 + 로그인 화면 재구성 + 자동 로그인 + 다크모드 버튼 버그 수정)
...
- ✅ 상단 헤더 — 최상단 우측에 🔔알림(`/notifications`)·⚙️설정(`/settings`) 아이콘(배경 없음). 그 아래 히어로 영역은 2026-07-08부터 구글 스마트 디스플레이 스타일 좌우 대칭: **왼쪽**(온도 40px + 날씨 이모지, 그 아래 날씨 상태 15px, 그 아래 지역명 "서울" 10px 자간 넓게) / **오른쪽**(시간 56px 얇은 웨이트 + AM/PM, 그 아래 날짜·요일 한 줄 12px, 오른쪽 정렬). 히어로 아래 [본인 아바타(18px, 표시 전용)+상태 텍스트]. "OO님 안녕하세요" 텍스트 없음
  - 상태 텍스트는 **루틴 기반 상태만** 표시(예: "쉬는 중", "업무 중") — 2026-07-08부터 오늘 일정 병기를 제거해 "오늘 뭐하지" 섹션과의 중복을 없앰(`home/page.tsx`의 `familyStatus` 계산에서 `targetedSchedule` 병합 로직 삭제)
- ✅ 오늘 뭐먹지 — 가로 스와이프 캐러셀(스크롤바 숨김, `.scrollbar-hide` 유틸리티), 오늘 등록된 끼니 전부 시간순 정렬, 메뉴명 22px(2026-07-08 26px에서 축소) + "태그·타입" 보조줄(끼니별 가상 시간 표기는 제거 — 외식일 때만 실제 `reservation_time`을 붙임) + "+ 사이드·참여자 이름" 보조줄. 점 인디케이터는 활성 시 honey 색 길쭉한 필. 카드 배경 없음 (버그 수정 이력은 `DEBUG_LOG.md` 참고)
- ✅ 오늘 뭐하지 — 각 행 `[시간(honey, tabular-nums, 고정폭)] [제목] [대상(흐림)]`, 헤어라인으로 위 섹션과 구분. 2026-07-08: [오늘]/[이번 주] 탭 토글 제거(이번 주 보기는 일정 탭 전담) — 오늘 일정만 최대 3개 표시하고, 초과 시 하단 "더보기"(11px, 오른쪽 정렬)로 `/schedule`로 이동. 일정 없으면 "오늘 등록된 일정이 없어요"
- ✅ 지금 우리 가족은 — 아바타 원 없이 텍스트만 `[이름(고정폭)] [상태] [이모지]`, **본인 제외**(본인 상태는 헤더에 이미 있음). 2026-07-08: 상태 텍스트에서 오늘 일정 병기 제거(위 상단 헤더와 동일 원칙, 루틴 상태만)
...
- ✅ 2026-07-08: 섹션 라벨(오늘 뭐먹지/오늘 뭐하지/지금 우리 가족은/스티커/장바구니) 앞에 14px 픽토그램 아이콘 추가 (`SectionLabel` 공용 컴포넌트, `mirror.label` 색 그대로 상속)
- ✅ 2026-07-08 섹션 내용 들여쓰기 — 라벨의 [아이콘 14px + gap-1.5(6px)]과 내용 시작점을 맞추기 위해 `--section-indent`(20px) 토큰 신설, `pl-section-indent`로 5개 홈 섹션(끼니/오늘 일정/가족 상태/스티커/장바구니) 콘텐츠 래퍼에 공통 적용
- ✅ 2026-07-08 텍스트 말줄임(ellipsis) 버그 수정 — flex 행 안의 `truncate` span이 `min-w-0` 없이 `flex-1`(혹은 아무 크기 지정도 없이)만 있으면 브라우저 기본값(`min-width: auto`)때문에 실제로는 잘리지 않고 밀려나가는 문제가 있었음. 홈(오늘 일정 행, 가족 상태, 게시판 미리보기 장바구니)과 일정 탭(월/주/연간 뷰 리스트), 공유 링크 표시 등 전반에 `min-w-0 flex-1` 조합을 일괄 적용
- ✅ 2026-07-08 홈 섹션 순서 커스터마이징(모바일 전용, 태블릿 3단 쇼케이스는 순서 변경 대상 아님) — 드래그 가능한 3개 위젯 단위: ① `meal`(오늘 뭐먹지 캐러셀), ② `today`(오늘 뭐하지 + 지금 우리 가족은을 2단으로 묶은 하나의 단위), ③ `board`(게시판 미리보기, 스티커+장바구니 2단). 섹션을 500ms 길게 누르면 편집 모드 진입(전체 섹션 `scale-0.97` + 각 섹션 왼쪽에 `IconGripVertical` 핸들 노출), 핸들을 드래그하면 `@dnd-kit/core`+`@dnd-kit/sortable`로 순서 변경. 편집 모드 종료는 빈 곳 탭 또는 상단 "완료" 버튼. 순서는 낙관적으로 즉시 반영되고, 서버 액션(`updateHomeLayout`, `home/actions.ts`)으로 `users.home_layout`(JSONB 배열, 예: `["meal","today","board"]`)에 저장 — 저장 실패해도 화면 순서는 그대로 유지하고 토스트만 띄움. 페이지 로드 시 `resolveHomeLayout()`(`src/lib/homeLayout.ts`)이 저장값에서 알려진 섹션 id만 남기고, 새로 추가될 미래 섹션은 자동으로 뒤에 이어 붙임(값이 없으면 기본 순서 `meal→today→board`). 컴포넌트: `src/components/home/HomeSections.tsx`
...
- ✅ 2026-07-08: 일정 탭 전용 AI 버튼(플레이스홀더, `AiAssistButton`)은 삭제하고 모든 탭에서 쓰는 플로팅 에이전트(`AgentLauncher`)로 대체됨 — 아래 "일정 파싱 에이전트" 섹션 참고

**일정 파싱 에이전트** (`agent/`, 플로팅 대화창) — 2026-07-08 신규
- ✅ 모든 탭(홈/식탁/일정/게시판) 우측 하단에 상시 노출되는 플로팅 버튼(`AgentLauncher`, honey 원형 52px + 스파클 아이콘) → 탭하면 화면 78% 높이 바텀시트 대화창(`AgentSheet`)이 슬라이드업. 핸들바를 위로 드래그하면 풀스크린, 아래로 드래그하거나 X를 누르면 닫힘
...
src/
  ...
  components/
    home/                  HomeHeader/MealSummaryCard/TodayEvents/FamilyStatusCard — 2026-07-07 전부 "스마트미러" 스타일로 재작성(카드 제거, mirror.* 토큰 사용). SectionLabel(2026-07-08 신규, 아이콘+라벨 한 줄 공용 컴포넌트), HomeSections(2026-07-08 신규, 모바일 홈 섹션 드래그앤드롭 순서 변경 — `@dnd-kit` 기반), BoardPreview(홈 미리보기 2열), ShoppingList(게시판 탭 전체 관리용, 홈에서는 더 이상 안 씀), BoardSection(게시판 탭, 60/40+댓글)
    food/                  MealCard, MealDetail, AddMealScreen, WeekCalendar
    schedule/              MonthView/WeekView/YearView, AddEventEntry(+picker+4개 Sheet), EventFilters, RoutineEditor, PlaceInput (2026-07-08: 일정 탭 전용 AiAssistButton 플레이스홀더는 삭제됨 — agent/ 아래 참고)
    agent/                 2026-07-08 신규: AgentLauncher(플로팅 버튼, 모든 탭), AgentSheet(바텀시트 대화창), ConfirmCards(확인 카드, 입력 소스 무관 재사용 가능하게 설계)
  ...
  lib/
    ...
    agentApi.ts               2026-07-08 신규: callAgent() — NEXT_PUBLIC_AGENT_API_URL의 /process-schedule 호출, AgentSchedule/AgentResponse 타입
