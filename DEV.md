...
## 마지막 업데이트: 2026-07-19 (디바이스 레이아웃 분기 개선 + PWA 매니페스트)
...
- 🔧 2026-07-18 자동 로그인 진단/수정 — "로그인 유지가 안 된다"는 제보를 코드 리딩으로 진단.
  ...
  **미확인**: 이 환경엔 브라우저가 없어 "브라우저 완전 종료 후 재접속 시 로그인 유지"를
  ...
  선택과 무관하게 라이브러리 기본값(약 400일 영속)으로 다시 덮어써질 가능성이 있음 —
  이번 수정 범위 밖의 별도 이슈로 남겨둠(고치려면 로그인 시점에 "세션 전용" 선택을 별도
  마커 쿠키로 남기고 미들웨어가 매 리프레시마다 그 마커를 보고 maxAge를 강제로 되돌리는
  로직이 추가로 필요함)
