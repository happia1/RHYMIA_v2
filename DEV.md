...
## 마지막 업데이트: 2026-07-11 (홈 화면/장바구니 UI 정리 — 위젯 4분할 + 100dvh 고정)
...
- ✅ 구성: 입력란(2026-07-11: 오른쪽에 마이크 아이콘 — `webkitSpeechRecognition` 지원 브라우저에서만 노출, `ko-KR`로 인식한 텍스트를 입력란에 채워넣음, 인식 중엔 honey 색) + 미구매 리스트(2026-07-11: 항목 행 `ShoppingItemRow` 전체가 탭 영역 — 탭하면 `toggleShoppingPurchased` 호출, 왼쪽 스와이프 또는 x로 삭제) + **"구매 완료" 아카이브**(구매 완료 항목을 `purchased_at` 날짜별로 그룹핑해 표시, 취소선)
