...
- ✅ 2026-07-19 크롭 편집 + 압축 공용화 — `src/components/ui/ImageCropSheet.tsx` 신규(캔버스 기반 자체 구현, 외부 라이브러리 없음). 원형 프레임(고정 1:1, `AvatarUploader`/`ManagedAvatarUploader`)과 사각 프레임(자유 비율 — 1:1/4:3/16:9 프리셋, `HomePhotoManager`) 두 모드를 지원하며, 드래그로 이미지 이동 + 휠/두 손가락 핀치로 확대·축소(Pointer Events로 마우스/터치 통합, 두 번째 포인터가 활성화되면 거리 변화로 핀치 배율 계산). "완료" 시 canvas에서 긴 변 1080px·JPEG 품질 0.9로 다시 인코딩해 반환하므로 이 크롭 단계 자체가 압축을 겸함(별도 `compressImage` 호출 불필요). 게시판 스티커/메모 이미지 첨부(`AddPostSheet.tsx`)는 크롭 없이 `compressImage`만 새로 추가 — 이제 프로필(본인/관리 멤버)·홈 화면 사진·끼니 사진·게시판 이미지 전부 예외 없이 압축을 거침. 진단 결과 managed 멤버 업로드 경로(`avatars/managed/{workspaceId}/{memberId}/...`)는 스토리지 정책(`add_managed_avatar_storage.sql`)의 경로 규칙과 이미 정확히 일치했고 그 SQL도 이미 재실행 안전(idempotent) — 코드 수정은 필요 없었고 실행 여부만 남은 상태(아래 TODO)
...
- ✅ 2026-07-11 "오늘 뭐먹지" 리뉴얼 — 등록 중심에서 메뉴 결정 경험으로 확장. **결정/추천 UI는 식탁 탭 전용**(같은 날 안에 홈에도 잠깐 뒀다가 바로 식탁 탭으로 옮김 — 홈은 "오늘 등록된 것만" 보여주는 상태판이라는 원칙으로 정리):
  ...
  - **"오늘의 제안"** 카드 캐러셀(`SuggestionSection.tsx`, `src/components/food/`) — 식탁 탭 페이지 하단에 상시 노출(끼니 등록 여부와 무관). 가로 스와이프 카드: 늘 먹던 메뉴 / 룰렛 바로가기("메뉴 랜덤 고르기", 탭하면 `MealDecisionSheet` 룰렛 모드로 바로 오픈). 홈의 위젯 순서 변경 시스템(`HomeSectionId`)에는 더 이상 포함되지 않음. **"추천 레시피" 카드는 2026-07-19에 여기서 빠져 아래 "레시피" 섹션으로 이동함(바로 아래 항목 참고)**
  ...
  - ✅ 2026-07-19 **태블릿 우측 탭 구조 재편** — 냉장고/장바구니 아코디언은 **세로(포트레이트) 태블릿에만** 남기고(접힘 요약을 냉장고 칸과 동일하게 "첫 2개 + 외 N개"로 통일), **가로(랜드스케이프)**는 아코디언 대신 상단 탭 [집에 뭐 있지 | 뭐 사야하지]로 바꿔 선택된 쪽이 우측 전체 높이를 그대로 씀(`FoodTabletHome.tsx`가 `useDeviceLayout()`의 `layout==='tablet-landscape'` 직접 분기, `DeviceLayoutSwitch`는 모바일/태블릿만 나누므로 이 안에서 추가로 나눔) — "뭐 사야하지" 탭의 기록 달력(`ShoppingTabbedPanel`)이 아코디언 압축으로 잘리지 않게 하려는 목적. 뭐 사야하지 영역에도 냉장고와 동일한 라운드 테두리 컨테이너(`rounded-2xl border`) 적용(이전엔 냉장고에만 있었음)
  - ✅ 2026-07-19 **레시피 재구성** — "오늘의 제안" 배너 안 "추천 레시피" 카드를 없애고, 식탁 탭 하단에 모바일·태블릿 공통 `RecipeSection.tsx`(신규) 신설 — 오늘의 추천 1건(식품안전나라), 레시피 노트 진입, 레시피 찾아보기(검색 시트)를 한 곳에 모음(전엔 태블릿에만 있던 "레시피 노트" 진입점 + 배너 카드가 따로였음). 끼니 추가 화면(`AddMealScreen.tsx`)의 "레시피 찾아보기"는 더 이상 시트를 그 자리에서 열지 않고 `/food?date=...&openRecipe=1`로 이동만 함(그 자리에서 채우던 `handleFillFromRecipe` 흐름은 삭제 — **의도적 트레이드오프**, 검색을 한 곳으로 합치는 대신 끼니 작성 도중 이어서 채우는 편의는 포기함). 대신 원래 검색 시트에 딸려 있던 "레시피 링크 붙여넣기"(유튜브 URL 직접 붙여넣기)는 별도 텍스트 버튼으로 끼니 추가 화면에 그대로 남김(이것만 사라지면 그 기능 자체가 화면에서 완전히 사라지므로). `RecipeSearchSheet.tsx`는 `onFillFromRecipe`/`onSaveBlogLink`/`onOpenLinkPaste`/`memo` 전부 옵션으로 바꿔 끼니 등록 화면 "안"(콜백 전달)과 식탁 탭 "레시피" 섹션 단독(콜백 없이 `selectedDate`만 — 내부 `RecipeDetailSheet`가 자동으로 "오늘 메뉴로 추가하기"→`/food/add` 이동 모드로 폴백) 양쪽에서 재사용되게 함
...
- ✅ 2026-07-19 **태블릿 월간 뷰 재구성** — 좌측 "축소판 달력"(도트만)을 버리고, 모바일 월간 뷰가 쓰던 달력 그리드(도트·기간 밴드 라인·라벨 규칙 전부)를 `src/components/schedule/MonthCalendarGrid.tsx`로 뽑아내 모바일·태블릿 양쪽이 그대로 재사용하도록 함(별도 재구현 없음 — `MonthView.tsx`는 이제 이 컴포넌트 하나만 다른 props로 두 번 호출). 월 표기(연월+이전/다음 화살표)는 태블릿에서만 좌측 칼럼 중앙 상단으로 옮기고(상단 공용 바는 태블릿일 때 중복 표시 안 함), 달력 아래 "오늘" 버튼 추가 — 지금 보는 달이 이번 달이 아니면 이번 달로 이동, 이미 이번 달이면 선택일만 오늘로 바꾸고 우측 패널을 스크롤 맨 위로 되돌림. 우측 패널은 날짜를 아직 한 번도 직접 고르지 않은 기본 상태일 때 오늘 일정/할 일을 보여주고 헤더에 "오늘 ·"를 덧붙임(`DaySheetContent`의 신규 `emphasizeToday` prop) — 아무 날짜나 탭하는 순간부터는 일반 날짜 표기로 전환됨. 주간 뷰(`WeekView.tsx`)는 원래부터 `useDeviceLayout` 분기가 없어 모바일·태블릿에서 이미 완전히 동일한 표(날짜|구분선|일정) 형태라 변경 없음
...
- ✅ 2026-07-19 태블릿 조정 — 메모·공지사항 우측 고정 패널의 좌(제목 리스트)우(본문) 비율을 `240px 고정`에서 **3:7**(`grid-cols-[3fr_7fr]`)로 변경(`BoardSection.tsx`). 상세 이미지(`NoticeDetailContent.tsx`)는 `object-cover`로 가로폭 꽉 채워 늘이던 것을 `object-contain`(원본 비율 유지, `max-h-56`은 그대로)으로 바꾸고, 탭하면 신규 `src/components/ui/ImageLightbox.tsx`(원본 크게 보기, 배경/닫기 버튼 아무 데나 탭하면 닫힘)가 뜨도록 함 — 모바일 팝업과 태블릿 우측 패널이 `NoticeDetailContent` 하나를 공유하므로 두 화면 모두 동일하게 적용됨
...
- [ ] 2026-07-19 이미지 업로드 개선(크롭+압축 공용화) + 게시판·일정·식탁 태블릿 조정 + 레시피 재구성 — 전부 `tsc --noEmit`/`next lint` 클린까지만 확인, 로그인된 브라우저가 없어 아래는 전부 실제 화면에서 직접 확인 필요:
  - `ImageCropSheet`의 드래그 팬/휠 줌(데스크톱)/두 손가락 핀치 줌(모바일·태블릿 실기기)이 실제로 자연스러운지, 프로필(원형 1:1)·홈 사진(사각 자유 비율, 1:1/4:3/16:9 전환) 양쪽 다 크롭 결과물이 기대한 영역을 정확히 잘라내는지
  - 게시판 태블릿 3:7 분할이 실제로 자연스러운 비율인지, 이미지 원본 비율 유지+라이트박스 확대가 잘 되는지
  - 일정 태블릿 좌측 달력이 모바일과 정말 동일하게 보이는지(도트·밴드·라벨), "오늘" 버튼이 다른 달을 보던 중에도 정확히 이번 달+오늘 선택으로 돌아오는지, 우측 패널 "오늘 ·" 라벨이 날짜를 탭하는 순간 정확히 사라지는지
  - 식탁 탭 가로 태블릿의 상단 탭 전환이 매끄러운지(특히 "뭐 사야하지" 탭의 기록 달력이 잘리지 않는지), 세로 태블릿 아코디언의 "첫 2개+외 N개" 요약이 냉장고/장바구니 양쪽에서 통일돼 보이는지
  - 레시피 섹션 재구성 후 끼니 추가 화면의 "레시피 찾아보기" → `/food?openRecipe=1` 이동이 실제로 검색 시트를 자동으로 연 채 도착하는지, "레시피 링크 직접 붙여넣기"가 여전히 정상 동작하는지
