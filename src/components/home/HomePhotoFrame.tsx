 ...
 * 라이트/다크 테마와 무관한 의도된 예외(사진이 없을 때 "순수 블랙 여백" 요구사항과도 같은
 * 맥락). 사진이 하나도 없으면 그 검정 배경만 남는다. */
export function HomePhotoFrame({ photoUrls }: { photoUrls: string[] }) {
  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center bg-black">
      ...
      />
      {photoUrls.length > 1 && (
        // 프레임 자체가 테마와 무관하게 항상 검정이라(위 주석 참고), 여기 점 색상도 앱의
        // 라이트/다크 토큰이 아니라 검정 배경 위에서 항상 또렷한 고정 회색을 그대로 쓴다.
