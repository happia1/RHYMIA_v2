...
/** 장바구니 시트의 "기록" 탭 — 이번 달 장보기 달력·합계, 품목/구매처 검색, 기록 직접 추가,
 * 개별 기록 상세(품목·영수증). 모바일 전역 장바구니 시트(GlobalShoppingSheet)와 태블릿 식탁
 * 탭 우측 하단 확장 패널(ShoppingTabbedPanel 경유) 양쪽이 그대로 공유한다. */
export function GroceryHistoryTab({ workspaceId }: { workspaceId: string }) {
    ...
    return (
      <div className="flex flex-col gap-3">
        <button
          ...
          className="flex items-center gap-1 self-start text-[16px] text-stone"
        >
          <IconChevronLeft size={16} />
          기록
        </button>
        <div className="flex flex-col gap-1 border-b border-border-light pb-3">
          <span className="text-[20px] font-medium text-ink">{openRun.place || "장보기"}</span>
          <span className="text-[14px] text-stone">
            {openRun.date.replaceAll("-", ".")} · {openRun.itemNames.length}개 품목
          </span>
          <span className="mt-1 text-[24px] font-medium text-honey">{won(openRun.amount)}</span>
        </div>
        <span className={LABEL_CLASS}>산 것들</span>
        <div className="flex flex-col">
          {openRun.itemNames.length === 0 && (
            <p className="py-2.5 text-[16px] text-[var(--text-muted)]">연결된 품목이 없어요</p>
          )}
          {openRun.itemNames.map((name, i) => (
            <div
              key={i}
              className={`py-2.5 text-[17px] text-ink ${i > 0 ? "border-t border-border-light" : ""}`}
            >
              {name}
            </div>
          ))}
        ...
        {openRun.receiptImageUrl ? (
          ...
          <p className="text-[16px] text-[var(--text-muted)]">등록된 영수증이 없어요</p>
        )}
        <button
          disabled
          className="mt-2 flex h-11 items-center justify-center rounded-xl border border-border-light text-[14px] text-stone/40"
        >
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-7 gap-y-1.5 border-b border-border-light pb-3 text-center">
        ...
        {cells.map((d, i) => {
          ...
          return (
            ...
            >
              <span className={`text-[16px] ${isSelected ? "font-medium text-honey" : "text-ink"}`}>
                {d}
              </span>
      ...
      <div className="flex items-baseline justify-between border-b border-border-light pb-3">
        ...
        <span className="text-[19px] font-medium text-honey">{won(monthTotal)}</span>
      </div>
      ...
      <Input
        ...
        className="h-10 px-0 text-[16px]"
      />

      {loadError && <p className="text-[14px] text-terra">{loadError}</p>}
      ...
      <div className="flex flex-col">
        ...
        {visibleRuns.map((r, i) => (
          ...
          >
            <span className="w-11 shrink-0 text-[14px] text-stone">
              {Number(r.date.slice(5, 7))}.{Number(r.date.slice(8, 10))}
            </span>
            <span className="min-w-0 flex-1">
              <span className="text-[17px] text-ink">{r.place || "장보기"}</span>
              <span className="block truncate text-[14px] text-[var(--text-muted)]">
                {r.itemNames.join(" · ")}
              </span>
            </span>
            <span className="shrink-0 text-[17px] text-ink">{won(r.amount)}</span>
          </button>
        ))}
      </div>
    </div>
