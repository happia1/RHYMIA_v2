/** 워크스페이스 멤버 표시 정보 — 타입/상수/순수 변환 함수만 여기 둔다(클라이언트에서도
 * 안전하게 import 가능). 실제 DB 조회(getWorkspaceMembers)는 서버 전용인
 * `@/lib/members.server`에 있다 — 같은 파일에 두면 그 파일의 `next/headers` 의존이
 * ManagedAvatarUploader 같은 클라이언트 컴포넌트까지 번들링돼 빌드가 깨진다. */
